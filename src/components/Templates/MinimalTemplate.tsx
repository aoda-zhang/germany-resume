...
export function MinimalTemplate() {
  ...
  const renderSection = (section: typeof visibleSections[0]) => {
    switch (section.type) {
        ...
        return (
          <header className="mb-5">
            <h1 className="font-light text-slate-900 mb-2" style={{ fontSize: '20pt' }}>
              <EditableText value={personalInfo.fullName} onChange={(v) => updatePersonalInfo({ fullName: v })} placeholder={t.name} className="font-light" />
            </h1>
      ...
      case 'experience':
        ...
        return (
          <section className="mb-5">
            <EditableLabel sectionType="experience" defaultLabel={tEditor.experience} className="font-medium text-slate-400 uppercase tracking-wider block mb-3" style={{ fontSize: '9pt' }} />
            <div className="space-y-3">
              {experience.map((exp) => (
                <div key={exp.id}>
                  <div className="flex justify-between items-baseline">
                    <h3 className="text-slate-900" style={{ fontSize: '10.5pt' }}>
                      <EditableText value={exp.position} onChange={(v) => updateExperience(exp.id, { position: v })} placeholder={t.position} />
                    </h3>
                    <span className="text-slate-400" style={{ fontSize: '8.5pt' }}>
                      ...
                      }} placeholder={t.startDate} className="text-slate-400" />
                    </span>
                  </div>
                  ...
                  {exp.techStack && <div className="mt-1 text-slate-400" style={{ fontSize: '8.5pt' }}>{exp.techStack}</div>}
      ...
      case 'education':
        ...
        return (
          <section className="mb-5">
            <EditableLabel sectionType="education" defaultLabel={tEditor.education} className="font-medium text-slate-400 uppercase tracking-wider block mb-3" style={{ fontSize: '9pt' }} />
            <div className="space-y-2">
              {education.map((edu) => (
                <div key={edu.id}>
                  <div className="flex justify-between items-baseline">
                    <h3 className="text-slate-900" style={{ fontSize: '10.5pt' }}>
                      <EditableText value={edu.school} onChange={(v) => updateEducation(edu.id, { school: v })} placeholder={t.school} />
                    </h3>
                    <span className="text-slate-400" style={{ fontSize: '8.5pt' }}>
      ...
      case 'projects':
        ...
        return (
          <section className="mb-5">
            <EditableLabel sectionType="projects" defaultLabel={tEditor.projects} className="font-medium text-slate-400 uppercase tracking-wider block mb-3" style={{ fontSize: '9pt' }} />
            <div className="space-y-3">
              {projects.map((proj) => (
                <div key={proj.id}>
                  <h3 className="text-slate-900" style={{ fontSize: '10.5pt' }}>
                    <EditableText value={proj.name} onChange={(v) => updateProject(proj.id, { name: v })} placeholder={t.projectName} />
                  </h3>
                  <p className="mt-1 text-slate-600 whitespace-pre-line" style={{ fontSize: '9pt' }}>
      ...
      case 'skills':
        ...
        return (
          <section className="mb-5">
            <EditableLabel sectionType="skills" defaultLabel={tEditor.skills} className="font-medium text-slate-400 uppercase tracking-wider block mb-3" style={{ fontSize: '9pt' }} />
      ...
      case 'languages':
        ...
        return (
          <section className="mb-5">
            <EditableLabel sectionType="languages" defaultLabel={tEditor.languages} className="font-medium text-slate-400 uppercase tracking-wider block mb-3" style={{ fontSize: '9pt' }} />
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-slate-600" style={{ fontSize: '9.5pt' }}>
              {languages.map((lang) => (
                <span key={lang.id}>
  ...
  return (
    <div className="bg-white shadow-lg" style={{ padding: '12mm 15mm', minHeight: '297mm', boxSizing: 'border-box', width: '100%' }}>
